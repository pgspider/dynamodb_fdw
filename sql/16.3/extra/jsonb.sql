...
SELECT pg_input_is_valid(x, 'jsonb') FROM pg_input_is_valid_tbl;
...
--Testcase 341:
CREATE FOREIGN TABLE test_jsonb_subscript ("id" text, test_json jsonb)
 SERVER dynamodb_server OPTIONS (table_name 'test_jsonb_subscript_text', partition_key 'id');
--Testcase 342:
...
--Testcase 345:
update test_jsonb_subscript set test_json['foo'] = '"baz"';
--Testcase 346:
select length(id), test_json[id] from test_jsonb_subscript;
...
insert into test_jsonb_subscript values('foo', '{"foo": "bar"}');
-- Hash primary key values must be under 2048 bytes, and range primary key values must be under 1024 bytes
-- then repeat 350 times instead of 500 times 
insert into test_jsonb_subscript
  select s, ('{"' || s || '": "bar"}')::jsonb from repeat('xyzzy', 350) s;
select length(id), test_json[id] from test_jsonb_subscript;
