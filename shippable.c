 ...
 *
 * IDENTIFICATION
 *	  contrib/dynamodb_fdw/shippable.c
 *
 *-------------------------------------------------------------------------
 */
 ...
 */
bool
dynamodb_is_builtin(Oid objectId)
{
	return (objectId < FirstGenbkiObjectId);
}

/*
 * dynamodb_is_shippable
 *
 * Is this object (function/operator/type) shippable to foreign server?
 */
bool
dynamodb_is_shippable(Oid objectId, Oid classId, DynamoDBFdwRelationInfo *fpinfo)
{
	ShippableCacheKey key;
	ShippableCacheEntry *entry;

	/* Built-in objects are presumed shippable. */
	if (dynamodb_is_builtin(objectId))
		return true;
	...
	if (!entry)
	{
		...
		bool		shippable = dynamodb_lookup_shippable(objectId, classId, fpinfo);
